...
public class BankTest {
    ...
    public void depositMoney(@RequestParam("accountNr") String accountNr, @RequestParam("amount") BigDecimal amount) {
        ...
        // addAmountToCurerntAccountBalance(accountNr, amount);
    }
    ...
    public void transferMoney(@RequestParam("fromAccount") String fromAccount,
        ...
        // reduceAmountFromCurerntAccountBalance(fromAccount, amount);
        //  addAmountToCurerntAccountBalance(toAccount, amount);
...
/*
    public void addAmountToCurerntAccountBalance(String accountNr, BigDecimal amountToAdd) {
        BigDecimal currentBalance = getAccountBalance(accountNr);
        BigDecimal newBalance = currentBalance.add(amountToAdd);
        updateAccountBalance(accountNr, newBalance);

    }

    public void reduceAmountFromCurerntAccountBalance(String accountNr, BigDecimal amountToReduce) {
        BigDecimal currentBalance = getAccountBalance(accountNr);
        if (currentBalance.compareTo(amountToReduce) < 0) {
            throw new RuntimeException("Not enough money to transfer");
        }
        BigDecimal newBalance = currentBalance.subtract(amountToReduce);
        updateAccountBalance(accountNr, newBalance);
    }
